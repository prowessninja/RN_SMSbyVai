  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        //...(userType !== 'Organisation Admin' ? { branch_id: branchId } : {}),
        branch_id: branchId,
        dob: formData.dob || '',
        education_details: [
          {
    ...
    <ScrollView style={styles.container}>
      ...
      {userType === '' ? (
        <View>
          ...
          {renderPicker(
            formData.group,
            v => { handleMainChange('group', v); setUserType(v); },
            (dropdowns.groups?.results?.map(s => s.name).filter(name => !['Organisation Admin', 'IoT User'].includes(name))) || [],
            'Select User Type'
          )}
